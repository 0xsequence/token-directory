...
async function checkExternalTokenLists() {
  ...
  try {
    ...
    if (
      !externalData.externalTokenLists ||
      !Array.isArray(externalData.externalTokenLists)
    ) {
      console.error(
        'Error: external.json does not contain an externalTokenLists array'
      )
      process.exit(1)
    }
    ...
    if (duplicateNames.length > 0) {
      console.error(
        `Error: Duplicate token list names found: ${duplicateNames.join(', ')}`
      )
      process.exit(1)
    }
    ...
    if (!fs.existsSync(externalDir)) {
      ...
      console.log(
        `Cleared ${existingFiles.length} files from _external directory`
      )
    }
    ...
    results.forEach(result => {
      if (result.status === 'fulfilled') {
        if ('error' in result.value) {
          ...
          console.log(
            `✅ ${result.value.name}: Successfully fetched, validated and saved JSON (${result.value.sizeInMB} MB)`
          )
    ...
    results.forEach(result => {
      if (
        ...
        'sizeInMB' in result.value &&
        result.value.sizeInMB
      ) {
    ...
    if (failures.length > 0) {
      ...
      console.log(
        `Total size of successful downloads: ${totalSizeMB.toFixed(2)} MB`
      )
      process.exit(1)
    } else {
      console.log(
